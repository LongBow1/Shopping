...
public class ShoppingForAppBll {
            ...
            if(goodInfo != null && goodInfo.getAddressDetailInfo() != null ){
                ...
                if(finalGoodsList1 != null && finalGoodsList1.getData() != null && finalGoodsList1.getData().getRows() != null){
                    if(finalNewGoodsList != null && finalNewGoodsList.getData() != null && finalNewGoodsList.getData().getRows() != null){
                        ShoppingForAppDTO.GoodsListDTO finalGoodsList = finalGoodsList1;
    ...
    public static void commitToBuyOrder(List<ToBuyGoodInfoAppDTO.ToBuyGoodAndAddressInfoDTO> toBuyGoodAndAddressInfos, String auth, String memberId) {
        StringBuilder operationInfo = new StringBuilder("");
        if(AutoShoppingEntryForApp.mapOrderedCounter.get(memberId) <= 0){
            ...
            toBuyGoodAndAddressInfos.forEach(toBuy -> {
                if(toBuy.getAddressDetailInfo() != null && toBuy.getToBuyGoodInfoList() != null && !toBuy.getToBuyGoodInfoList().isEmpty()){
                    toBuy.getToBuyGoodInfoList().forEach(buyGood -> {
                        ...
                        if(canCommitOrder){
                            commitOrderFutures.add(executorServiceForCommitOrder.submit(() -> RequestBllForApp.commitOrderDetail(toBuy, buyGood, RequestBllForApp.getCommitPostEntity(buyGood, toBuy.getAddressDetailInfo(), buyGood.getToBuyNum()), auth, 0, memberId)));
                        }
                    });
                    //有多个优惠同时走购物车下单
            ...
            try {
                ...
                if(commitOrderError != null){
                    ...
                    while (Optional.ofNullable(commitOrderError.getCode()).orElse(0) == 80005 && count<6){
                        commitOrderError = JSONObject.parseObject(commitOrderForShoppingCarV2(commitPostEntity, auth, memberId),ShoppingForAppDTO.CommitOrderErrorDTO.class);
                        count++;
    ...
    private static String commitOrderForShoppingCarV2(StringEntity commitPostEntity, String auth, String memberId) {
        String operationInfo = "";
        String orderInfo = "";
        try {
            orderInfo = doPostForShoppingCar(commitOrderUrlForApp, commitPostEntity, auth);
        ...
        if (toBuyGoodAndAddressInfos != null && !toBuyGoodAndAddressInfos.isEmpty()) {
            try {
                ...
                if(alreadyBuyLocalNos != null && !alreadyBuyLocalNos.isEmpty()){
                    String messageResult = WeChatBll.sendMessage("order bqj success:" + intendToBuyGoods.stream().filter(intend -> alreadyBuyLocalNos.contains(intend.getLocalNo())).findFirst().orElse(new ToBuyGoodInfoAppDTO.ToBuyGoodAndAddressInfoDTO()).getDesc(), Arrays.asList(qqOpenId, zzOpenId, zzjjOpenId));
