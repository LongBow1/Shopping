...
public class AutoShoppingEntryForWeChatApp {
    ...
    static String intendOrderFilePrefixNotCommit = "readToBuyFileForMaGe";
    /**
     * 链接测试
     *
     * @param auth
    ...
    public static boolean deleteIntendOrders(String memberId, String localNos) {
        ...
        String readToBuyFile = intendOrderFilePrefixNotCommit+memberId+".txt";
    ...
    public static String cancelOrder(String orderNos, String auth, String memberId) {
        ...
        //local records
        List<String> localOrderNos = mapAlreadyBuyGoodAndAddressInfos.get(memberId).stream().map(ToBuyGoodInfoAppDTO.ToBuyGoodAndAddressInfoDTO::getToBuyGoodInfoList).flatMap(Collection::stream).collect(Collectors.toList()).stream().map(ShoppingForAppDTO.GoodDataStockDetailDTO::getOrderNo).collect(Collectors.toList());
        try {
            ...
            orderList.stream().filter(item -> item != null && !item.trim().isEmpty() && (localOrderNos.isEmpty() || localOrderNos.contains(item))).forEach(orderNo -> {
    ...
    public static String cancelOrder(String toPayOrderUrl, String cancelOrderUrl, String orderNos, String auth, String memberId) {
        AtomicReference<String> result = new AtomicReference<>("success");
        List<String> orderList = Arrays.asList(orderNos.split("\\,"));
        //local records
        List<String> localOrderNos = mapAlreadyBuyGoodAndAddressInfos.get(memberId).stream().map(ToBuyGoodInfoAppDTO.ToBuyGoodAndAddressInfoDTO::getToBuyGoodInfoList).flatMap(Collection::stream).collect(Collectors.toList()).stream().map(ShoppingForAppDTO.GoodDataStockDetailDTO::getOrderNo).collect(Collectors.toList());
        try {
            if(orderList.stream().filter(item -> item != null && !item.trim().isEmpty()).count() == 0){
                List<ToPayOrderDTO> toPayOrderDTOS = getToPayOrderList(auth, memberId, toPayOrderUrl);
                orderList = toPayOrderDTOS.stream().map(ToPayOrderDTO::getOrderNo).collect(Collectors.toList());
            }
            orderList.stream().filter(item -> item != null && !item.trim().isEmpty() && (localOrderNos.isEmpty() || localOrderNos.contains(item))).forEach(orderNo -> {
                String reqResultInfo = RequestBllForApp.doPost(cancelOrderUrl,RequestBllForApp.getCancelOrderForMagePostEntity(orderNo, "diswant"), auth);
                if(reqResultInfo != null){
                    ShoppingForAppDTO.CommitOrderDTO commitOrder = JSONObject.parseObject(reqResultInfo, ShoppingForAppDTO.CommitOrderDTO.class);
                    if(commitOrder != null && !commitOrder.getMessage().equals("成功")){
                        result.set(result.get() + orderNo + commitOrder.getMessage());
    ...
    public static String startAutoShopping(String auth, String memberId, String localNos) {
        ...
        for(;;){
            ...
            try {
                ...
                ShoppingForWeChatAppBll.buildToBuyGoodInfo(mapToBuyGoodAndAddressInfos.get(memberId), auth);
                ShoppingForWeChatAppBll.commitToBuyOrder(mapToBuyGoodAndAddressInfos.get(memberId), auth, memberId);
                ShoppingForWeChatAppBll.removeAlreadyBuyAndToPayGood(mapToBuyGoodAndAddressInfos.get(memberId), mapAlreadyBuyGoodAndAddressInfos.get(memberId), mapIntendToBuyGoodInfos.get(memberId));
                FileOperation.writeFile(JSONObject.toJSONString(mapIntendToBuyGoodInfos.get(memberId)), readToBuyFile);
            }catch (Exception ex){
                ex.printStackTrace();
